...
class Tickers(multiprocessing.Process):
    ...
    def run(self):
        with open(self.file, 'r') as f:
            ...
            self.collector.put(self.tick_volatility)
...
if __name__ == '__main__':
    ...
    tickers = [Tickers(file=file, collector=collector) for file in file_path_generator('trades')]
    for ticker in tickers:
        ticker.start()
    for ticker in tickers:
        ticker.join()
    while not collector.empty():
        name, volatility = collector.get()
        res.append((name, volatility))
    print_result(res)
